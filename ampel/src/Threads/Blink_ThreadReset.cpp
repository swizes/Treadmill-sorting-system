...
Blink_ThreadReset* Blink_ThreadReset::instance_ = NULL;
 ...
 */
Blink_ThreadReset* Blink_ThreadReset::getInstance() {
	static pthread_mutex_t mtx_ = PTHREAD_MUTEX_INITIALIZER;

	if (instance_ == NULL) {
		pthread_mutex_lock(&mtx_);
		if (instance_ == NULL) {
			instance_ = new Blink_ThreadReset();
		}
 ...
 */
Blink_ThreadReset::Blink_ThreadReset(void ) {
 ...
 */
Blink_ThreadReset::~Blink_ThreadReset() {
 ...
 */
void Blink_ThreadReset::execute(void*){
	int run = 1;
	while(run){
		if(countBlink < 0){
			led->setReset(1);
 ...
 */
void Blink_ThreadReset::shutdown(){
    cout << "Blink_Thread shutdown" << endl;
}

void Blink_ThreadReset::setLED(int countBlink, int time){
